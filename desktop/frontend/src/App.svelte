<script>
  ...
  let transferRequest = null; // persisted save directory
  ...
  onMount(async () => {
    ...
    EventsOn("transfer_request", (payload) => {
      try {
        const req = typeof payload === "string" ? JSON.parse(payload) : payload;
        if (!req || !req.id) return;
        transferRequest = req;
        playSound("blip");
      } catch (e) { console.error("transfer_request parse error", e); }
    });

    EventsOn("transfer_request_timeout", (payload) => {
      try {
        const data = typeof payload === "string" ? JSON.parse(payload) : payload;
        const timedOutId = data?.id;
        if (transferRequest && transferRequest.id === timedOutId) {
          toast("⏱ Transfer request timed out", "warn");
          transferRequest = null;
        }
      } catch (e) { console.error("transfer_request_timeout parse error", e); }
    });
  ...
  async function approveTransfer() {
    ...
    transferRequest = null;
    playSound("blip");
    try { await ApproveTransfer(id); }
    catch (e) { toast("❌ Approve failed: " + e, "error"); }
  }
  async function rejectTransfer() {
    ...
    transferRequest = null;
    playSound("click");
    try { await RejectTransfer(id); toast("🚫 Transfer rejected", "warn"); }
    catch (e) { toast("❌ Reject failed: " + e, "error"); }
  }
...
{#if transferRequest}
  <div class="tr-backdrop">
    <div class="tr-dialog">
      <h2 class="tr-dialog__title">// INCOMING_TRANSFER</h2>
      <div class="tr-rows">
        <div class="tr-row"><span class="tr-row__label">FILE</span><span class="tr-row__val">{transferRequest.filename}</span></div>
        <div class="tr-row"><span class="tr-row__label">SIZE</span><span class="tr-row__val">{formatSize(transferRequest.sizeBytes)}</span></div>
        <div class="tr-row"><span class="tr-row__label">TYPE</span><span class="tr-row__val">{transferRequest.mimeType || "unknown"}</span></div>
        <div class="tr-row"><span class="tr-row__label">FROM</span><span class="tr-row__val tr-row__ip">{transferRequest.senderIP}</span></div>
      </div>
      <div class="tr-actions">
        <button class="action-btn action-btn--red" on:click={rejectTransfer}>✕ REJECT</button>
        <button class="action-btn action-btn--cyan" on:click={approveTransfer}>✓ ACCEPT</button>
      </div>
    </div>
  </div>
{/if}
